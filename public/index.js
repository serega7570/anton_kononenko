...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите фамилию клиента:');
    const nomer = prompt('Введите список препаратов:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новую фамилию клиента:', oldTaskName[0]);
    const newNomer = prompt('Введите новый список препаратов:', oldTaskName[1]);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Заказ клиента '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить заказ';
